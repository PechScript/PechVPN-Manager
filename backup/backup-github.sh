...
LLatest=`date`
Get_Data () {
git clone https://github.com/PechScript/BackupVPS.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
cd /root/user-backup
git config --global user.email "pechweb950@gmail.com" &> /dev/null
git config --global user.name "PechScript" &> /dev/null
rm -fr .git &> /dev/null
git init &> /dev/null
git add . &> /dev/null
git commit -m m &> /dev/null
git branch -M PechVPN &> /dev/null
git remote add origin https://github.com/PechScript/BackupVPS.git
git push -f https://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@github.com/PechScript/BackupVPS.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/PechScript/BackupVPS/PechVPN/$NameUser/$NameUser.zip"
sleep 1
...
function restore(){
cd
read -rp "Enter Name File Your Backup or Link  : " -e NameUser

cekdata=$(curl -sS https://raw.githubusercontent.com/PechScript/BackupVPS/PechVPN/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
...
mkdir -p /root/backup
wget -q -O /root/backup/backup.zip "https://raw.githubusercontent.com/PechScript/BackupVPS/PechVPN/$NameUser/$NameUser.zip" &> /dev/null
